...
```{r Point_estimate, eval=F,echo=T}
dat = data.frame(
  K_s1 = 0.25,
  M_K_s1 = 1.05,
  L50_Linf_s1 = 0.74,
...
In this example we will process some example data, train an indicator and estimate stock status using it. The ETest package comes with an example dataset 'Some_data' for these worked examples. The dataset has catches, CPUE indices, mean length, life history and selectivity information for three fleets simultaneously fishing three fleets. 

To make creating time series data features easier, ETest has a function ts_features(). 
...
plot(names(Catch),Catch) # what do these catch data look like?
Catch_f = ts_features(Catch, "C")  # lets capture the rel, s5, s10 and s20 features
