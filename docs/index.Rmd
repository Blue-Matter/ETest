...
LH_sel_data = data.frame(K_s1 = Some_data$K[stock],                    # combine life history and selectivity info
                       ...
                       L5_L50_s1_f1 = Some_data$L5[stock,fleet]/Some_data$L50[stock,fleet],   # shortest length at 5% selectivity relative to L50
                       LFS_L50_s1_f1 = Some_data$LFS[stock,fleet]/Some_data$L50[stock,fleet]) # length at 95% selectivity relative to L50
...
In general fisheries stock assessment has failed to incorporate information across multiple species. This is a missed opportunity. For example if the denominator of CPUE (effort) is comparable in trend among two species, their catch ratio informs their *relative* depletion. This means that depletion on one stock starts to be informed by data about depletion on the other stock. As the number of stocks increases (that adhere to this assumption of approximately comparable effort trends) an increasibly narrow set of depletion conditions can prevail. To demonstrate this lets see how well only catch data can inform stock status across three species: 
...
msdat = c(unlist(Catch_f1),unlist(Catch_f2),unlist(Catch_f3))
names(msdat) = paste0(names(msdat),"_s",rep(1:3,each=5),"_f1")
